...
from quantish.gate import Gate, DelayGate, FredkinGate
...
from quantish.config_space import default_switches, default_wires, WIRES, SWITCH_WIRES, OTHER, STRAIGHT, SWAPPED, \
    WorldState, GateState, ExecutionStage
...
class Simulation:
    def __init__(self, config):
        ...
        self.selector =  self.config.get('probability_threshold', {}).get('selector', 0)
        ...
        if self.selector is not None:
            if self.selector == -1:
                ...
                # self.selector = ((random.random() * PI() * 2) % (PI()/2)).cos**2
    ...
    def run_one_phase(self, execution_phase):
        for gate in execution_phase.gates:
