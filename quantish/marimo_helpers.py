...
def measure_many(gate: FredkinGate, controls=None, uppers=None, lowers=None,
    ...
    for control in controls:
        output_dict[ctrl_pos].append(
            Sink(ctrl_pos,presence_threshold=presence_threshold, initial_values=[control]))
        ...
        for (out_pos, dest, outs, msg) in zip(
                ...
                ('upper', 'lower')):
            if outs is not None:
                sink = Sink(out_pos, presence_threshold=presence_threshold)
                sinks[out_pos] = sink
                outputs = []
                for pval in outs:
...
def load_selected_model(configs, selected, config_ui):
    if configs is not None:
        if selected is not None and len(selected) > 0:
            ...
            config['winner_take_all'] = config_ui['winner_take_all']
