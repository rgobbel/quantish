    ...
    def __init__(self, config):
        ...
        for obname in self.order:
            if obname in self.gates.keys():
                ...
                print(f'{gate=}')
                cval = gate.port_value('control')
                uval = gate.port_value('upper')
                lval = gate.port_value('lower')
                print(f'{gate.name}: control:{cval}, upper:{uval}, lower: {lval}')
        ...
        for obname in self.order:
            if obname in self.particles.keys():
                ...
                for wire in WIRES: # look up sources and destinations for all wires in this gate
                    ...
                    inputs[wire] = gate.port_value(wire)
                        # inputs[wire] = []
                ...
                if inputs['control']:
                    ...
                    gate.control = merged_control
                    swap = 'swapped' if enough(merged_control.probability, 0) else 'straight'
                ...
                # log.info(f'      control= {merged_control}')
                for wire in SWITCH_WIRES:
                ...
                for input_wire in SWITCH_WIRES:
                    ...
                    if switch_results[input_wire]:
                        ...
                        gate.outputs[input_wire][swap] = switch_results[input_wire]
