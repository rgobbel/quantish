...
G1_ANGLE = qn.qify('rad(30)')
G2_ANGLE = qn.qify('rad(120)') #G1_ANGLE
G3_ANGLE = 0
G4_ANGLE = G3_ANGLE
# G5_ANGLE = qn.qify('rad(20)')
# G6_ANGLE = G5_ANGLE
# G5_ANGLE = 0
# G6_ANGLE = qn.qify('rad(45)')
G5_ANGLE = qn.qify('pi/8')
        ...
        for g in sim.gates.values():
            for wire in WIRES:
                ...
                if g.outputs[wire]:
                    ...
                    result[out_pos] = None
        coupled = True #sim.gates['g4'].outputs['upper'] is not None
        if coupled:
            coupled_count += 1
            ...
            if g5up != g6up: discrepancy_count += 1
        # result = sim.propagate_weights()
        for k, v in result.items():
            if v is not None:
                histogram[k] += 1
            # print(f'{k}: {v}')
        if i < N_ITER - 1: del sim
